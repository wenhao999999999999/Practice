...
// 写状态转移方程思路：明确状态，也就是原问题和子问题中会变化的变量（一般是函数入参）-> 明确dp函数的定义，返回值一般是题目要求我们计算的量 -> 明确选择，也就是导致「状态」产生变化的行为 。

// 核心条件：必须具备 最优子结构（整体最优解可由子问题的最优解组合而成），并且通常还伴随 重叠子问题（相同子问题会被多次计算）。

// 求解思路：通过 状态转移方程，递归或迭代地枚举所有可能的子问题解，然后从中选取最优解。

// 特点：系统性、全面性 → 算法复杂度高，但能保证得到全局最优解。
